...
function WorkerDetail({ worker, onRemoveWorker, setSelectedWorker, assignments, onChangeOfStatus, updateWorkerState }: WorkerDetailProps) {
  ...
  return <Paper withBorder p="lg"><Stack>
    ...
    <Select label="Availability" value={worker.status} data={['available', 'sick', 'vacation', 'holiday', 'inactive']} onChange={(value) => onChangeOfStatus({ ...worker, status: (value ?? 'available') as WorkerStatus })} />
    {worker.status !== 'available' && <Text size="sm" c="orange">Changing this worker to unavailable removes all of their current assignments.</Text>}
